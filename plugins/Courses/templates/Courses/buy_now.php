                ...
                <table class="table table-bordered checkout-table">
                    ...
                    <tbody>
                        <tr>
                            <td>
                                <?= $course->title ?>
                                <p><?= $course->subject->title ?> | <?= $course->grading_type->title ?></p>
                                </td>
                            <td><h4><?= $this->Number->currency($course->price, 'INR') ?></h4></td>
                            <td>
                                <?php $discP = !empty($course->discount_price) ? round(($course->discount_price / $course->price) * 100, 2) : 0; ?>
                                <h4><?php echo $discP ?>%</h4>
                            </td>
                            <td class="text-right"><h3><?= $this->Number->currency(($course->price - $course->discount_price), 'INR') ?></h3></td>
                        </tr>
                    </tbody>
                </table>
