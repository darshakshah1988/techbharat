                ...
                <table class="table table-bordered checkout-table">
                    ...
                    <tbody>
                        ...
                        foreach ($carts as $cart):  
                            ...
                            $totalAmount = $cart->plan->price - $cart->plan->discount_price;
                        ...
                        <tr>
                            ...
                            <td>
                                <?= $cart->has('course') ? $this->Html->link($cart->plan->title, ['controller' => 'Courses', 'action' => 'view', 'plugin' => 'Courses', $cart->plan->id]) : '' ?>
                                <p><?= $cart->package->package_name ?> | <?= $cart->plan->plan_name ?></p>
                            </td>
                            <td><h4><?= $this->Number->currency($cart->plan->price, 'INR') ?></h4></td>
                            <td>
                                <?php $discP = !empty($cart->plan->discount_price) ? round(($cart->plan->discount_price / $cart->plan->price) * 100, 2) : 0; ?>
                ...
                <div class="col-sm-12 text-center mt30 mb40">
                    ...
                    <?php if (!empty($carts->toArray())): ?>
                        <div class="col-sm-4 col-sm-offset-4 mt20 mb40">
                            <?= $this->Html->link('<span>CHECK</span>OUT', ['controller' => 'Orders', 'action' => 'packagecheckout'], ['class' => 'cdm-right-buy-btn', 'escape' =>  false]) ?>
                        </div>
                    <?php endif; ?>
...
echo $this->Html->script(['/assets/plugins/jquery-loading-overlay-master/dist/loadingoverlay.min','https://cdnjs.cloudflare.com/ajax/libs/jquery-confirm/3.3.4/jquery-confirm.min.js'
],['block' => true]);
