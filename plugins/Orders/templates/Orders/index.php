...
<div class="new-header-style-1">
    <div class="container">
        <div class="col-sm-12 text-center">
            ...
            <p>Teaching Bharat's Course order history</p>
        </div>
    </div>
</div>
                    ...
                    <?php if (!empty($orders->toArray())):
                        ...
                        foreach ($orders as $order): 
                            ...
                            foreach($order->order_courses as $course){
                                $order_courses[] = $this->Html->link($course->course->title, ['controller' => 'Courses', 'action' => 'view', 'plugin' => 'Courses', $course->course->id], ['target' => '_blank']);
                            }
                            ?>
                     <tr>
                            <td><?= $this->Number->format($i) ?>.</td>
                            <td><?= $this->Html->link(h($order->invoice_no), ['action' => 'invoice', $order->id],['escape' => false, 'target' => '_blank', 'data-toggle'=>'tooltip','alt'=>__('View Invoice'),'title'=>__('View Invoice')]) ?></td>
                            <?php if($this->request->getAttribute('identity')->role == "teacher"){ ?>
                                <td>
                                    <?= $order->user->name ?>
                                </td>
                            <?php } ?>
                            <td><?= implode(", ", $order_courses) ?></td>
