...
try {
    ...
    Configure::load('setting', 'default');
...
if(!$isCli){
    $setting = \Cake\ORM\TableRegistry::getTableLocator()->get('Listings.Listings')->find('listId',['domain'=> Configure::read('Setting.domainUrl')])->cache(function ($q) {
                 return 'setting-domain-' . md5(serialize($q->clause('where')));
     })->first();
     if(!empty($setting)){
         Configure::write('LISTING_ID',$setting->id);
         $listing = $setting->toArray();
         $listing['theme_name'] = "CommonTheme";
         Configure::write('Tech',$listing);
     }else{
         die("comming soon...");
     }
 }

$this->getEventManager()->on(new \App\Event\TeachersAction());